...
from evaluation_T import evaluate_ranking, evaluate_train
from interactions import Interactions
from utils import *
# debug
# import pydevd_pycharm
# pydevd_pycharm.settrace('172.25.165.188', port=3931, stdoutToServer=True, stderrToServer=True)
...
class Recommender(object):
    ...
    def __init__(self,
        ...
        self.usermap = {}
        self.itemmap = {}
        for key, value in self.model_args.usermap.items():
            self.usermap[value] = int(key)
        for key, value in self.model_args.itemmap.items():
            self.itemmap[value] = int(key)
    ...
    def fit(self, train, test, verbose=False):
        ...
        evaluate_train(self, train, train)
...
if __name__ == '__main__':
    ...
    # data arguments
    # parser.add_argument('--train_root', type=str, default='/content/drive/MyDrive/MIA/Recommender/caser_pytorch-master/datasets/ml-1m_Tmember_train')
    # parser.add_argument('--test_root', type=str, default='/content/drive/MyDrive/MIA/Recommender/caser_pytorch-master/datasets/ml-1m_Tmember_test')
    # parser.add_argument('--train_root', type=str, default='/content/drive/MyDrive/MIA/Recommender/caser_pytorch-master/datasets/beauty_Tmember_train')
    # parser.add_argument('--test_root', type=str, default='/content/drive/MyDrive/MIA/Recommender/caser_pytorch-master/datasets/beauty_Tmember_test')
    parser.add_argument('--train_root', type=str, default='/content/drive/MyDrive/MIA/Recommender/caser_pytorch-master/datasets/book_Tmember_train')
    parser.add_argument('--test_root', type=str, default='/content/drive/MyDrive/MIA/Recommender/caser_pytorch-master/datasets/book_Tmember_test')
    ...
    model_parser.add_argument('--usermap', type=dict, default={})
    model_parser.add_argument('--itemmap', type=dict, default={})
    ...
    model_config.usermap = train.user_map
    model_config.itemmap = train.item_map
    # print("train.itemmap:", train.item_map, flush=True)
    # itemmap = open("datasets/ml-1m_itemmap", 'w')
    # usermap = open("datasets/ml-1m_usermap", 'w')
    # for key, value in model_config.itemmap.items():
    #     itemmap.write(str(key)+'\t'+str(value)+'\n')
    # for key, value in model_config.usermap.items():
    #     usermap.write(str(key) + '\t' + str(value) + '\n')
