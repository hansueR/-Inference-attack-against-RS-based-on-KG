...
#     print("用户的ID取值范围：{} 到 {}".format(min_id, max_id))
...
def get_items(user_id, movie_index_item2entity):
    ...
    # file = '/content/drive/MyDrive/Recommendation-system-based-on-ripplenet/data/book/BX-Book-Ratings.csv'
    ...
    # for line in lines:
    #     array = line.strip().split(';')
    #     user_id_str = array[0][1:-1]  # 去掉首尾的双引号
    #     book_isbn = array[1][1:-1]  # 去掉首尾的双引号
    #     book_rating = int(array[2][1:-1])  # 去掉首尾的双引号并将评分转换为整数
    
    #     if int(user_id_str) == user_id:  # 查中
    #         item_set.discard(book_isbn)  # 从备选集中剔除所查中的ISBN
    #         n_rating += 1
