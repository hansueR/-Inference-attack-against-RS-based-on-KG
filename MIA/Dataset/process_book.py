...
def removeShortSeq(data):
    ...
    data = data[np.in1d(data['UserID'], seqLen[seqLen > 0].index)]
    return data
...
# anonymize & remove users and items with less than 20 interactions records
...
def step0(filename, filename1):
    ...
    data = data[np.in1d(data.ItemID, itemLen[itemLen > 19].index)]
    data = removeShortSeq(data)
...
def step1(filename):
    ...
    path = "data/processed_book/"
    num_target = num_shadow = num_vector = 0

    for userID, userSeq in userGroup:
        rand = random.randint(1, 3)
        if rand == 1:
            userSeq.to_csv(path + 'book_vector_pre', sep=',', index=False, mode='a', header=None)
            num_vector = num_vector + 1
        elif rand == 2:
            userSeq.to_csv(path + 'book_shadow_pre', sep=',', index=False, mode='a', header=None)
            num_shadow = num_shadow + 1
        else:
            userSeq.to_csv(path + 'book_target_pre', sep=',', index=False, mode='a', header=None)
...
if __name__ == '__main__':
    filename = "/content/drive/MyDrive/Recommendation-system-based-on-ripplenet/data/book/ratings_first.txt"
    filename1 = "data/processed_book/ratings_after_step0.txt"
    step0(filename, filename1)
    step1(filename1)
    fr_target = 'book_target_pre'
    fr_shadow = 'book_shadow_pre'
    fr_vector = 'book_vector_pre'

    fw_target = 'book_target'
    fw_shadow = 'book_shadow'
    fw_vector = 'book_vector_pre1'
    ...
    fw_vector1 = 'book_vector'
    fw_itemdict = 'book_itemDict'
